...
import io.quarkus.workshop.superheroes.fight.HeroesVillainsNarrationWiremockServerResource;
import io.quarkus.workshop.superheroes.fight.InjectGrpcWireMock;
import io.quarkus.workshop.superheroes.fight.InjectWireMock;
...
import io.quarkus.workshop.superheroes.fight.client.FightToNarrate.FightToNarrateLocation;
...
class FightResourceIT {
  ...
  void helloNarrationFail() {
    ...
    expectedFight.location = DEFAULT_LOCATION;
		...
		var fight = get("/api/fights/{id}", getAndVerifyAllFights().get(0).getId())
			.then()
			.statusCode(OK.getStatusCode())
			.contentType(JSON)
