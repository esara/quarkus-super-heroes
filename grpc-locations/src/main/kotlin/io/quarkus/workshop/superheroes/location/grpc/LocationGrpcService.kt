package io.quarkus.workshop.superheroes.location.grpc
...
import io.quarkus.workshop.superheroes.location.grpc.LocationsGrpc.LocationsImplBase
import io.quarkus.workshop.superheroes.location.mapping.LocationMapper
import io.quarkus.workshop.superheroes.location.service.LocationService
...
class LocationGrpcService(private val locationService: LocationService) : LocationsImplBase() {
  ...
  override fun getRandomLocation(request: RandomLocationRequest?, responseObserver: StreamObserver<Location>?) {
	...
	private fun returnLocationOrError(location: io.quarkus.workshop.superheroes.location.Location?, responseObserver: StreamObserver<Location>?) {
		if (location != null) {
